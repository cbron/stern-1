...
func NewContainerState(stateConfig []string) (ContainerState, error) {
	...
	for _, p := range stateConfig {
		if p == RUNNING || p == WAITING || p == TERMINATED {
			...
			return []string{}, errors.New("containerState should only include 'running', 'waiting', or 'terminated'")
		}
	}
	if len(containerState) == 0 {
		return []string{}, errors.New("containerState must include 'running', 'waiting', or 'terminated'")
	}
